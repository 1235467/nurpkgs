...
for attr_name in "${PKG_ATTR_NAMES[@]}"; do
    ...
    if [[ "$attr_name" == "cudatoolkit" || "$attr_name" == "feishu" || "$attr_name" == "wechat" || "$attr_name" == "cider3" ]]; then
        ...
        KEEPALIVE_PID=$!
        if DRV_OUTPUT_PATH=$(nix build --no-link --print-out-paths --substituters "https://dead_link/nurpkgs" --trusted-public-keys "nurpkgs:gtUQ6um2j/MF+9nEDbsFa8S1dEct5YpaclN5/cHZFFE=" "${flake_ref}"); then
            attic push nurpkgs $DRV_OUTPUT_PATH
            kill $KEEPALIVE_PID
        else
            kill $KEEPALIVE_PID
